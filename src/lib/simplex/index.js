export class Simplex {
  constructor() {
    ...
    this.init = function (modelo) {
      ...
      this.restricoes = [
        ...modelo["restricoes"].map((restriction) => [...restriction]),
      ];
    ...
    this.modelo = function () {
      ...
      if (this.problema === "Maximize") source = "Maximizar";
      if (this.problema === "Minimize") source = "Minimizar";
      source += "\n`z = ";
      var primeiro = true;
      for (var i = 0; i < this.objetivo.length; i++) {
        if (this.objetivo[i] === "0") continue;
        if (!primeiro) source += this.objetivo[i] > 0 ? " + " : " ";
        else primeiro = false;
        source +=
          this.objetivo[i] === "1"
            ? " x_" + (i + 1) + " "
      ...
      if (this.restricoes.length !== 0) {
        for (i = 0; i < this.restricoes.length; i++) {
          ...
          for (var j = 0; j < this.objetivo.length; j++) {
            ...
            if (!primeiro) source += this.restricoes[i][j] > 0 ? " + " : " ";
            else primeiro = false;
            source +=
    ...
    this.grandeM = function () {
      ...
      for (var i = 1; i < this.objetivo.length; i++)
        if (this.objetivo[i] > max) max = this.objetivo[i];
      var M = 99999 * max;
      ...
      else this.tabela[0] = this.objetivo;
      ...
      for (var i = 0; i < this.lower.length; i++) {
        ...
        if (this.lower[i] !== 0) {
          ...
          for (var j = 0; j < this.lower.length; j++) {
            if (j === i) this.tabela[this.tabela.length - 1].push(1);
            else this.tabela[this.tabela.length - 1].push(0);
          }
          this.relacoes.push(">=");
      ...
      for (var i = 0; i < this.relacoes.length; i++) {
        if (this.relacoes[i] === "<=") {
          ...
          for (var j = 0; j < this.tabela.length; j++) {
            if (j === i + 1) this.tabela[j].push(1);
            else this.tabela[j].push(0);
          }
        } else {
    ...
    this.duasFases = function () {
      ...
      //Criando tabela basica
      for (var i = 0; i < this.objetivo.length; i++) this.tabela[0].push(0);
      for (var i = 0; i < this.restricoes.length; i++)
        this.tabela[i + 1] = this.restricoes[i];
    ...
    this.duasFases2 = function () {
      ...
      if (tabela[0][tabela[0].length - 1] !== 0) {
        ...
        for (var idx = 0; idx < this.artificiais.length; idx++) {
          ...
          for (var i = 1; i < this.tabela.length; i++) {
            if (this.tabela[i][j] === 1 && um === -1) um = i;
            ...
            //Se artificial da base zero
              ...
              for (var j = 0; j < tabela[0].length; j++) {
                if (tabela[0][j] === 0) {
                  ...
                  var basica = true;
                  for (var i = 1; i < this.tabela.length; i++) {
                    if (this.tabela[i][j] === 1 && umLinha === -1) umLinha = i;
                    else if (this.tabela[i][j] !== 0) {
                      basica = false;
                      break;
                    }
      ...
      for (var i = 0; i < this.objetivo.length; i++)
        if (this.problema === "Maximize") this.tabela[0][i] = -this.objetivo[i];
        else this.tabela[0][i] = this.objetivo[i];
      var tam = this.tabela[0].length;
      for (var i = this.objetivo.length; i < tam; i++) this.tabela[0][i] = 0;
    ...
    this.iteracao = function (entra, sai) {
       ...
       */
      var res;
      if (this.terminou() && this.execucaoFinal) res = this.solver.tabela;
      else {
    ...
    this.resultado = function (nIteracao) {
      ...
      for (var j = 0; j < this.tabela[0].length - 1; j++) {
        ...
        for (var i = 0; i < lin; i++) {
          if (tabela[i][j] === 1)
            if (linhaBase === -1) linhaBase = i;
            else basica = false;
          else if (tabela[i][j] !== 0) basica = false;
        }
  ...
  */
      if (this.pivos[nIteracao]) return this.pivos[nIteracao];
      else return [NaN, NaN];
    };
  ...
  constructor() {
    ...
    this.terminou = function () {
      ...
      for (var j = 0; j < this.tabela[0].length - 1; j++) {
        ...
        var basica = true;
        for (var i = 1; i < this.tabela.length; i++) {
          if (this.tabela[i][j] === 1 && !um) um = true;
          else if (this.tabela[i][j] !== 0) {
            basica = false;
            break;
          }
    ...
    this.escolheVariavel = function () {
      ...
      if (idxEntra === -1) {
        for (var i = 0; i < this.tabela[0].length - 1; i++)
          if (
            this.tabela[0][i] < this.tabela[0][idxEntra] ||
      ...
      if (idxEntra !== -1) {
        ...
        for (var i = 1; i < this.tabela.length; i++) {
          if (this.tabela[i][idxEntra] > 0) {
            var ri =
              this.tabela[i][this.tabela[i].length - 1] /
              this.tabela[i][idxEntra];
      ...
      if (degenerado === rmin && rmin > 0) {
        ...
        for (var i = 1; i < this.tabela.length; i++) {
          if (
            rmin ===
            this.tabela[i][this.tabela[i].length - 1] / this.tabela[i][idxEntra]
          ) {
            idxSai.push(i);
          }
        }
    ...
    this.iteracao = function (entra, sai) {
      ...
      if (this.terminado) return this.tabela;

      this.nIteracoes++;

      //Caso tenha acontecido uma solucao ilimitada ou um loop infinito
      if (
        (isNaN(sai) && !Array.isArray(sai)) ||
        this.nIteracoes === this.limiteIteracoes ||
        isNaN(entra)
      ) {
        if (
          isNaN(sai) &&
        ...
        if (
          isNaN(entra) &&
          this.tipoRes.indexOf("Solução inexistente. ") === -1
        )
